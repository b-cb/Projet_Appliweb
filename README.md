...
classDiagram
    ...
    %% Relations JPA (Associations)
    Utilisateur "1" --> "*" Joueur : @OneToMany (participations)
    Partie "1" --> "4" Joueur : @OneToMany (joueurs)
    
    Joueur "*" --> "*" Carte : @ManyToMany (cartesEnMain)
    Pli "*" --> "4" Carte : @ManyToMany (cartes Jouees)
    
    Partie "1" --> "*" Pli : @OneToMany (plis)
    Joueur "1" --> "*" Pli : @OneToMany (remporte)
    
    Partie "1" --> "*" Enchere : @OneToMany (encheres)
    Joueur "1" --> "*" Enchere : @ManyToOne (annonceur)
    
    Utilisateur "1" --> "*" MessageChat : @OneToMany (messages)
    Partie "1" --> "*" MessageChat : @OneToMany (chatBox)
    
    Utilisateur "1" --> "*" Invitation : @OneToMany (envoyées/reçues)
    Partie "1" --> "*" Invitation : @OneToMany (cible)
